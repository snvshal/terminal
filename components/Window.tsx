...
const Window: React.FC<WindowProps> = ({ title, children, onClose, initialPosition }) => {
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-0 right-0 w-2 h-2 cursor-ne-resize" onMouseDown={(e) => handleMouseDown(e, 'resize', 'ne')}></div>
      <div className="absolute bottom-0 right-0 w-2 h-2 cursor-se-resize" onMouseDown={(e) => handleMouseDown(e, 'resize', 'se')}></div>
      <div className="absolute bottom-0 left-0 w-2 h-2 cursor-sw-resize" onMouseDown={(e) => handleMouseDown(e, 'resize', 'sw')}></div>
      <div className="absolute top-0 left-0 w-2 h-2 cursor-nw-resize" onMouseDown={(e) => handleMouseDown(e, 'resize', 'nw')}></div>
      <div className="absolute top-0 left-0 right-0 h-1 cursor-n-resize" onMouseDown={(e) => handleMouseDown(e, 'resize', 'n')}></div>
      <div className="absolute bottom-0 left-0 right-0 h-1 cursor-s-resize" onMouseDown={(e) => handleMouseDown(e, 'resize', 's')}></div>
      <div className="absolute top-0 bottom-0 left-0 w-1 cursor-w-resize" onMouseDown={(e) => handleMouseDown(e, 'resize', 'w')}></div>
      <div className="absolute top-0 bottom-0 right-0 w-1 cursor-e-resize" onMouseDown={(e) => handleMouseDown(e, 'resize', 'e')}></div>
    </div>
  )
}
